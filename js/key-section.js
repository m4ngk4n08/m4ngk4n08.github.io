function checkKey(key) {
    ...
    if (!window.DatabaseModule || !window.DatabaseModule.db) {
        ...
        document.getElementById("section-result").innerHTML = "Error: Database not loaded. Please try again later.";
        return;
    }
    ...
    try{
        ...
        const res = db.exec("select * from encrypted_keys where key = ?", [encryptedKey]);
        if(res.length <= 0){
        ...
        db.run("update encrypted_keys set decrypted = 1 where key = ?", [encryptedKey]);
